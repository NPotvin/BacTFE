...
%
% Template d'article avec programmes (en français)
%
% Yves ROGGEMAN - octobre 2017 - LuaLaTeX
%
\documentclass[a4paper,twoside,10pt,english,french]{article}   % ,twocolumn
%---                           % Encodage pour LuaLaTeX (= full unicode)
% \usepackage{fontspec}
\usepackage[no-math]{fontspec} % Si unicode-math
...
<\href{mailto:yves.roggeman@ulb.ac.be}{\tt yves.roggeman@ulb.ac.be}>}
...
%---                             Polices STIX-2 "par défaut"
\setmainfont[Scale=1.1]{STIX Two Text}   % NB. ni Hebrew ni Arabic
\setmathfont{STIX Two Math}    % Nécessite unicode-math
% Polices Google
\begin{comment}
\setsansfont[Scale=MatchUppercase]{Fira Sans}   % NB. ni Hebrew ni Arabic
% \setmonofont{Fira Mono}
\setmonofont[Ligatures=TeX]{Fira Code}   % Alternative avec ligatures
\newfontfamily{\Hebrfont}[Script=Hebrew]{Alef}
\DeclareTextFontCommand{\textHebr}{\Hebrfont}
...
\newfontfamily{\Arabfont}[Script=Arabic,Scale=1.1]{KacstOne}
\DeclareTextFontCommand{\textArab}{\Arabfont}
...
\end{comment}
% Polices Linux Libertine
\begin{comment}
\setmainfont[Scale=1.15]{Linux Libertine O}   % NB. pas d'Arabic
\setsansfont[Scale=MatchUppercase]{Linux Biolinum O}   % NB. pas d'Arabic
% \setmonofont{Linux Libertine Mono O}   % Sans bold !
\setmonofont[Scale=MatchUppercase]{Hack}
\newfontfamily{\Arabfont}[Script=Arabic,Scale=1.15]{Noto Naskh Arabic}
\DeclareTextFontCommand{\textArab}{\Arabfont}
\renewcommand*{\textArabic}[1]{\begingroup\Arabfont\textdir TRT#1\endgroup}
\renewenvironment{Arabic}{\Arabfont\textdir TRT\pardir TRT\bodydir TRT}{}
\end{comment}
% Polices GNU (Free UCS Outline Fonts)
\begin{comment}
\setmainfont{FreeSerif}[%        NB. pas de SmallCaps
  Scale=1.2,
  UprightFeatures={SmallCapsFont={STIX Two Text}},
...
\setsansfont{FreeSans}[%         NB. pas de SmallCaps
  Scale=MatchUppercase,
  UprightFeatures={SmallCapsFont={Fira Sans}},
  BoldFeatures={SmallCapsFont={Fira Sans Bold}},
  ItalicFeatures={SmallCapsFont={Fira Sans Italic}},
  BoldItalicFeatures={SmallCapsFont={Fira Sans Bold Italic}},
...
\end{comment}
% Polices DéjàVu (ex BitStream Vera Fonts)
% \usepackage[TT={Scale=MatchUppercase}]{dejavu-otf}   % at least v. 0.02 04/10/2017
% \begin{comment}
\setmainfont{DejaVuSerif}[%      NB. pas de SmallCaps
  Scale=0.92,
  Extension=.ttf,
  UprightFont=*,
  BoldFont=*-Bold,
  ...
  UprightFeatures={Letters=SmallCaps,SmallCapsFont=STIX2Text-Regular.otf},
  BoldFeatures={Letters=SmallCaps,SmallCapsFont=STIX2Text-Bold.otf},
  ItalicFeatures={Letters=SmallCaps,SmallCapsFont=STIX2Text-Italic.otf},
  BoldItalicFeatures={Letters=SmallCaps,SmallCapsFont=STIX2Text-BoldItalic.otf},
  SmallCapsFeatures={Letters=SmallCaps,Scale=MatchUppercase},
  ]
\setsansfont{DejaVuSans}[%       NB. pas de SmallCaps
  Scale=0.92,
  Extension=.ttf,
  UprightFont=*,
  BoldFont=*-Bold,
  ...
  UprightFeatures={Letters=SmallCaps,SmallCapsFont=FiraSans-Regular.otf},
  BoldFeatures={Letters=SmallCaps,SmallCapsFont=FiraSans-Bold.otf},
  ItalicFeatures={Letters=SmallCaps,SmallCapsFont=FiraSans-BookItalic.otf},
  BoldItalicFeatures={Letters=SmallCaps,SmallCapsFont=FiraSans-BoldItalic.otf},
  SmallCapsFeatures={Letters=SmallCaps,Scale=MatchUppercase},
  ]
...
%
\setmathfont{DejaVuMathTeXGyre.ttf}   % Nécessite unicode-math
% \end{comment}
%---
\renewcommand{\familydefault}{\sfdefault}  % Pour polices sans empattement
%---
% \usepackage[frenchmath,unicodeminus]{mathastext}   % pas avec DéjàVu !
\ifdefstring{\familydefault}{\sfdefault}%
{\newfontfamily{\CJKfont}{Noto Sans CJK JP}[Script=CJK]}%
{\newfontfamily{\CJKfont}{Noto Serif CJK JP}[Script=CJK]}%
\DeclareTextFontCommand{\textCJK}{\CJKfont}   % reste horizontal !
%---
% \let\luatexpardir=\pardir      %%% Pour luabidi buggé (<- Omega, anciennement renommé)
% \let\luatextextdir=\textdir    %%% Pour luabidi buggé (<- Omega, anciennement renommé)
% \let\luatexbodydir=\bodydir    %%% Pour luabidi buggé (<- Omega, anciennement renommé)
% \let\luatexpagedir=\pagedir    %%% Pour luabidi buggé (<- Omega, anciennement renommé)
% \let\luatexmathdir=\mathdir    %%% Pour luabidi buggé (<- Omega, anciennement renommé)
% \let\luatexboxdir=\boxdir      %%% Pour luabidi buggé (<- Omega, anciennement renommé)
% \usepackage{luabidi}           % Pour langues sinistroverses
% Pour langues sinistroverses (LuaTeX)
% NB. Aleph Directions: TLT (latin), TRT (arabic), RTT (cjk), LTL (mongolian)
\newcommand*{\RL}[1]{\begingroup\textdir TRT#1\endgroup}
\newcommand*{\LR}[1]{\begingroup\normalfont\textdir TLT#1\endgroup} % Revient en police "normale"
...
\newenvironment{RTT}[1][\baselineskip]{\begin{minipage}[t][\baselineskip]{#1}%
  \CJKfont\textdir RTT\pardir RTT\bodydir RTT}{\end{minipage}}   % vertical !
%
\usepackage[localmarks,babelshorthands,quiet]{polyglossia}   % localmarks => \markboth etc. + babelshorthands => : ; ? ! 
\setdefaultlanguage{french}      % En français, s.v.p.
\setotherlanguage[variant=usmax]{english}   % avec anglais 2d
% \setdefaultlanguage[variant=usmax]{english}   % En anglais
% \setotherlanguage{french}      %  avec français 2d
% \setotherlanguage[variant=ancient]{greek}
\usepackage{csquotes}            % Format des citations et guillement
\DeclareQuoteStyle{french}%        Paramètres pour dito
  {\guillemotleft~}{~\guillemotright}%
  {\textquotedblleft}{\textquotedblright}
\usepackage{geometry}          % Dimension des marges etc.
\geometry{                     % Paramètres pour dito
  inner=25mm,outer=20mm,
  top=25mm,bottom=25mm,head=15pt,
...
\graphicspath{{$HOME/Images/Logos/}{$HOME/}}
...
\usepackage{caption}           % Titres des figures et tables plus corrects
...
\newcommand{\lstcap}[1]{#1} % Pour titrer un programme (cf. \caption)
...
% \renewcommand{\qed}{\hspace*{1em}\hfill\ensuremath{\blacksquare}}  % QUOD ERAT DEMONSTANDUM (U+220E)
% \renewcommand{\qedsymbol}{\ensuremath{\blacksquare}}  % QUOD ERAT DEMONSTANDUM (U+220E)
\renewcommand{\qed}{\hspace*{1em}\hfill\ensuremath{\QED}}  % QUOD ERAT DEMONSTANDUM (U+220E) (nécessite unicode-math)
\renewcommand{\qedsymbol}{\QED}    % QUOD ERAT DEMONSTANDUM (U+220E) (nécessite unicode-math)
...
\indent ſt
st sp ct cp    \\*
\indent{\addfontfeatures{Ligatures={Historic,Rare}} ſt st sp ct cp} \\*
\indent ﬅ  ﬆ

\subsection{Autres alphabets}

Grec: \enquote{\rmfamily Εὐκλείδης} ou \enquote{\sffamily Εὐκλείδης} ou \enquote{Εὐκλείδης} ou \enquote{\bfseries Εὐκλείδης}.% Erreur d'espace !
...
CJK: \og\textCJK{錢天問}\fg ou \og\textbf{\textCJK{錢天問}}\fg ou Hello!

\begin{RTT}% Warning, car texte trop long
錢天問
\end{RTT}
\begin{RTT}[3em]
錢天問
...
Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Morbi sed justo. Cras venenatis rhoncus eros. Ut dictum elit vitae diam. Praesent in velit. \mymarginpar{In illo tempore erat fortissimus vir}Proin aliquam nulla eget nisl. Donec sed diam ac sem hendrerit mollis. \textsf{Suspendisse id justo.}\\
Suspendisse id justo.\\
Duis mollis, risus quis gravida dapibus, magna tortor sodales mauris, et ultricies magna orci a eros. Aliquam ac nisi. Aliquam nonummy placerat leo. In gravida. Aliquam erat volutpat. Phasellus euismod mauris non justo. Mauris ante ligula, semper pellentesque, sollicitudin nec, varius at, mauris. Mauris nec metus nec quam egestas congue.
...
où \todo{Origine?}$\gamma\approx0.5772156649$ est la \emph{constante d'Euler}.\qed
